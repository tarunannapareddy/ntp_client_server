...
func (c *ntpClient) start() error {
	...
	delayPlot.Title.Text = "NTP Delay vs Iterations from Local Server"
	...
	offsetPlot.Title.Text = "NTP Offset vs Iterations from Local Server"
...
func (c *ntpClient) sendRequest() (float64, int64, error) {
	...
	for i := 0; i < maxretries; i++ {
		...
		if opErr, ok := err.(*net.OpError); ok && opErr.Timeout() {
			fmt.Printf("Timeout occurred on attempt %d, retrying...\n", i+1)
			continue // Move to the next iteration for retry
		}
		if err != nil {
	...
	return 0, 0, fmt.Errorf("maximum retries (%d) reached for sending NTP request", maxretries)
